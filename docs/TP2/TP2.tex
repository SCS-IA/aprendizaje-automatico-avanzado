...
\section{Fragmentación con BPE}

\begin{frame}{BPE}
	\justifying
	
	Para el caso de BPE (Byte Pair Encoding) el corpus y el vocabulario se inicia con los tokens como símbolos primarios (letras, números, etc). Luego:
	\begin{center}
		\begin{itemize}
			\item Se van encontrando coincidencias de mayor cantidad de tokens unidos (priorizando tokens más largas).
			\item Se agrupan (merge) los tokens de mayor ocurrencia $N$ cantidad de veces.
		\end{itemize}
		
		\vspace{0.1cm}
		
		\begin{tikzpicture}[node distance=0.9cm and 0.6cm, every node/.style={font=\small}]
			\node[draw, fill=blue!10, rounded corners] (a1) {l};
			\node[draw, fill=blue!10, rounded corners, right=of a1] (a2) {a};
			
			\draw[->, thick, green!60!black] (a1.south) .. controls +(down:0.5cm) and +(down:0.5cm) .. (a2.south);
			
			\node[below=1.6cm of a2, draw, fill=green!10, rounded corners] (new1) {la};
